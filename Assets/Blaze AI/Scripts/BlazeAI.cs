...
public class BlazeAI : MonoBehaviour 
{
    ...
    void VisionCheck()
    {
        ...
        for (int i=0; i<visionCollNum; i++) {
            ...
            // check for alert tag only if AI isn't in attack state or going to cover
            if (state != State.attack && state != State.goingToCover) {
    ...
    public float CalculateCornersDistance(Vector3 destination)
    {
        NavMesh.CalculatePath(ValidateYPoint(transform.position), ValidateYPoint(destination), NavMesh.AllAreas, path);
